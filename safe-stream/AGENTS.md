...
The `Wrapper` class is marked `@internal`; the public API is the `nette.safe://` protocol used through native PHP file functions.

### Key Implementation Details

**File locking strategy:**
- Read operations (plain `r` mode only): `LOCK_SH` (shared lock) - multiple readers allowed
- All other modes, including `r+`: `LOCK_EX` (exclusive lock) - single writer, blocks all readers
- Append mode tracking: `$startPos` records file size before writing to enable rollback on errors (it stays 0 for all other modes)

**Write error handling:**
- `$writeError` flag tracks failed or incomplete writes (e.g., disk full)
...
- For reads: if the file is empty, releases and re-acquires the lock up to 100 times (another thread may be mid-write); after that the open succeeds anyway and the empty file is read
...
- `SafeStream.stress.phpt`: Concurrent access simulation (`@multiple 5` annotation runs 5 processes in parallel)
