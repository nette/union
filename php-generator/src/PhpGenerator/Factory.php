...
final class Factory
{
	...
	private function loadMethodBodies(\ReflectionClass $from): array
	{
		...
		$class = $nodeFinder->findFirst($stmts, fn (Node $node) => ($node instanceof Node\Stmt\Class_ || $node instanceof Node\Stmt\Trait_) && $node->namespacedName->toString() === $from->name);
	...
	private function loadFunctionBody(\ReflectionFunction $from): string
	{
		...
		$function = $nodeFinder->findFirst($stmts, fn (Node $node) => $node instanceof Node\Stmt\Function_ && $node->namespacedName->toString() === $from->name);
