...
class RequestFactory
{
	...
	private function useNonstandardProxy(Url $url, &$remoteAddr, &$remoteHost): void
	{
		...
		if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
			$xForwardedForWithoutProxies = array_filter(explode(',', $_SERVER['HTTP_X_FORWARDED_FOR']), fn (string $ip): bool => !array_filter($this->proxies, fn (string $proxy): bool => filter_var(trim($ip), FILTER_VALIDATE_IP) !== false && Helpers::ipMatch(trim($ip), $proxy)));
