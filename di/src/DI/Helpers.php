...
final class Helpers
{
	...
	public static function sortByConstraints(array $items): array
	{
		...
		$match = fn(string $target, string $dir): array => $target === '*'
			? array_keys(array_filter($items, fn($item) => !in_array('*', (array) ($item[$dir] ?? []), true)))
			: array_keys(array_filter($items, fn($item) => $item['owner'] && is_a($item['owner'], $target)));
