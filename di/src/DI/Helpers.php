...
final class Helpers
{
	 ...
	 * @throws Nette\InvalidArgumentException
	 */
	private static function expandString(string $var, array $params, bool|array $recursive = false): mixed
	{
					...
					if (is_array($val) && array_key_exists($key, $val)) {
						...
						if ($val instanceof DynamicParameter || $val instanceof Statement) {
							...
							foreach (explode('.', $part) as $i => $key) {
								$key = var_export($key, return: true);
								$val .= $i ? "[$key]" : "($key)";
							}
		...
		if ($php) {
			...
			$res = array_map(fn($val): string => $val instanceof DynamicParameter
					? (string) $val
					: var_export((string) $val, return: true), $res);
