...
use Nette;
use Nette\Schema;
use function array_diff_key, array_filter, array_keys, array_map, array_merge, array_values, assert, count, implode, is_array, key, sprintf, strtolower;
...
class Compiler
{
	...
	public function compile(): string
	{
		...
		// Register runs again for ServicesExtension's hook, registered only after processExtensions()
		foreach ([Phase::Register, Phase::Discover, Phase::Modify] as $phase) {
			$this->runPhase($phase);
		}
	...
	/**
	 * Processes extensions: validates config, calls register() and runs the Setup phase.
	 * Extensions are ordered by their Setup hooks; ServicesExtension goes last, once all
	 * definitions are visible. New extensions may be added during Setup, hence the loop.
	 */
	...
	/**
	 * Validates extension config and calls its register().
	 */
	...
	/**
	 * @deprecated handled internally by compile()
	 * @internal
	 */
