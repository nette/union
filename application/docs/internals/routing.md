...
`Nette:`**. `withModule($m)` creates a nested list whose `module` is `$m.':'` (a
whole group under a module), which is distinct from the per-route `<module>` mask
parameter above. `warmupCache` is *invoked* from `RoutingExtension::afterCompile`
