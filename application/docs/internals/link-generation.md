...
`LinkGenerator::createRequest` (~180 lines) is the densest, most trap-laden code in
...
| mode | absolute URL? | flash `_fid`? | notes |
|---|---|---|---|
| `link` | relative (unless `absoluteUrls`) | no | the normal case |
| `redirect` | absolute | yes* | `Component::redirect` |
| `forward` | absolute | yes* | no missing-`isLinkable` enforcement |
...
links outside a running presenter.
