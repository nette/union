...
`completeTemplate()` fills the template variables and, **only if the template has no
file set**, calls `findTemplateFile()` **then** — so template lookup happens at send
time, not when the template object is created. Changing the view is possible right
